        ...
        <div className="min-w-0 flex-1">
          ...
          {state.txHash && (
            ...
            >
              ...
              {state.txHash.slice(0, 8)}ΓÇª{state.txHash.slice(-6)} Γåù
            </a>
          )}
        </div>
      </div>
...
export default function AMPDemoSimulated() {
  ...
  const runPipeline = useCallback(async (file: File) => {
    ...
    try {
      // ΓöÇΓöÇ Phase 1 ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      setPhase(1, { status: 'active', detail: 'Reading image bytesΓÇª' });
      ...
      setPhase(1, { status: 'active', detail: 'Computing SHA-256ΓÇª' });
      const buf = await file.arrayBuffer();
      const hash = await sha256Hex(buf);
      setMediaHash(hash);
      await sleep(400);

      setPhase(1, { status: 'active', detail: 'Generating ECDSA P-256 key pairΓÇª' });
      ...
      await sleep(500);
      setPhase(1, { status: 'done', detail: `SHA-256 computed ┬╖ ECDSA P-256 signed ┬╖ Device key: ${dkHash.slice(0, 12)}ΓÇª` });

      // ΓöÇΓöÇ Phase 2 ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      setCurrentPhase(2);
      setPhase(2, { status: 'active', detail: 'Requesting World ID biometric verificationΓÇª' });
      await sleep(1200);
      const nullifier = '0x' + Array.from(randomBytes(32)).map(b => b.toString(16).padStart(2, '0')).join('');
      setPhase(2, { status: 'simulated', detail: `Nullifier bound to device key. ${nullifier.slice(0, 14)}ΓÇª` });

      // ΓöÇΓöÇ Phase 3 + 4 ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      setCurrentPhase(3);
      setPhase(3, { status: 'active', detail: 'Connecting wallet ΓåÆ Base SepoliaΓÇª' });
      ...
      if (provider) {
        try {
          ...
          if (alreadyTrusted) {
            ...
            setPhase(3, { status: 'active', detail: 'Sending registerDeviceKey() txΓÇª' });
            const tx = await vr.registerDeviceKey(dkHash);
            setPhase(3, { status: 'active', detail: 'Awaiting confirmationΓÇª', txHash: tx.hash });
            await tx.wait(1);
            setPhase(3, { status: 'done', detail: 'DeviceKeyRegistered confirmed on Base Sepolia.', txHash: tx.hash });
          }

          setCurrentPhase(4);
          setPhase(4, { status: 'active', detail: 'Encoding media hash as bytes32ΓÇª' });
          await sleep(300);
          const mediaBytes32 = ('0x' + hash).padEnd(66, '0').slice(0, 66) as `0x${string}`;
          setPhase(4, { status: 'active', detail: 'Calling requestVerification()ΓÇª' });
          const reqTx = await vr.requestVerification(dkHash, mediaBytes32, hexlify(toUtf8Bytes('')), 1n);
          setPhase(4, { status: 'active', detail: 'Awaiting VerificationRequested eventΓÇª', txHash: reqTx.hash });
      ...
      // ΓöÇΓöÇ Phase 5 ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      setCurrentPhase(5);
      setPhase(5, { status: 'active', detail: 'DON fetching C2PA manifest from IPFSΓÇª' });
      await sleep(1400);
      setPhase(5, { status: 'active', detail: 'Running verify_truth.js ΓÇö validating proofsΓÇª' });
      await sleep(1100);
      setPhase(5, { status: 'simulated', detail: 'DON consensus reached ┬╖ VerifiedReal emitted.' });

      // ΓöÇΓöÇ Phase 6 ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      setCurrentPhase(6);
      setPhase(6, { status: 'active', detail: 'CCIP encoding cross-chain messageΓÇª' });
      await sleep(1000);
      setPhase(6, { status: 'active', detail: 'Broadcasting badge to Arbitrum SepoliaΓÇª' });
      await sleep(1200);
      setPhase(6, { status: 'simulated', detail: '"Verified Real" BadgeIssued on Arbitrum Sepolia.' });

      // ΓöÇΓöÇ Manifest ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      const manifest = {
        '@context': 'https://c2pa.org/specifications/specifications/1.3/schema/claim.schema.json',
        type: 'c2pa.claim',
        claim_generator: 'Deepfake Slayer / AMP v0.1 ΓÇö RVR LLC',
        instance_id: `urn:uuid:${crypto.randomUUID()}`,
        created_at: new Date().toISOString(),
        media: { name: file.name, size: file.size, type: file.type, sha256: hash },
        hardware_layer: { key_algorithm: 'ECDSA-P256-SHA256', device_key_hash: dkHash, ecdsa_sig: sig.slice(0, 24) + 'ΓÇª' },
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 pt-20 sm:pt-28 lg:pt-32 pb-16 sm:pb-20">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-6 sm:mb-8">
          ...
          <p className="text-zinc-400 text-sm sm:text-base max-w-xl leading-relaxed">
            Take a photo ΓÇö watch it get <strong className="text-white">SHA-256 hashed</strong>, ECDSA signed,
          ...
          <div className="flex flex-wrap items-center gap-2 mt-4">
            ...
            {walletAddr && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-[10px] font-mono text-emerald-400">
                ...
                {walletAddr.slice(0, 6)}ΓÇª{walletAddr.slice(-4)}
              </span>
            )}
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 sm:gap-6">
          ...
          {/* ΓöÇΓöÇ LEFT: Capture card (dynamic header) ΓöÇΓöÇ */}
          <motion.div
          ...
          >
            ...
            <AnimatePresence mode="wait">
              {done ? (
                /* ΓöÇΓöÇ VERIFIED REAL badge ΓöÇΓöÇ */
                <motion.div
                  key="verified"
                ...
                /* ΓöÇΓöÇ Active phase status ΓöÇΓöÇ */
                <motion.div
                  ...
                  <div className="relative z-10 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-zinc-400 mt-0.5">
                        {phases[currentPhase]?.detail || 'ΓÇª'}
                      </p>
                    </div>
                  </div>
                </motion.div>
              ) : (
                /* ΓöÇΓöÇ Idle header ΓöÇΓöÇ */
                <motion.div
                  key="idle"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
            ...
            <div className="px-4 pb-4 flex flex-col gap-2.5">
              ...
              {/* Progress bar ΓÇö lives right here below the button */}
              <AnimatePresence>
                {(running || done) && (
                  <motion.div
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <div className="mt-4 flex items-start gap-2.5 px-4 py-3 rounded-xl bg-white/3 border border-white/6">
            ...
            <p className="text-[10px] text-zinc-500 leading-relaxed">
              <strong className="text-yellow-400">Demo disclaimer:</strong> ECDSA keys are ephemeral ΓÇö discarded on refresh.
              Phases 3ΓÇô4 require a funded wallet on Base Sepolia testnet. Contract addresses are live on testnets.
            </p>
          </div>
        </motion.div>
